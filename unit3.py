...
# compute integrand of distance formula
    ...
    # Rectangle rule with z=1
    def distance_rectangle(self, z, n):

        c = constants.speed_of_light / 1000.0  # Speed of light in Mpc/s
        dz = z / n
        integral = 0.0
        for i in range(n):
            z_i = i * dz 
...
# Return a string describing the model 
    ...
    def __str__(self):
        """Return a human-readable description of the cosmology.
        This is used by print() and str() to show the model parameters.
        """
        return f"Cosmology(H0={self.H0}, omega_m={self.omega_m}, omega_lambda={self.omega_lambda}, omega_k={self.omega_k})"
...
#def graph2():
    """Plot integrand curves for various omega_m by creating a new Cosmology
    instance for each omega_m. This differs from the previous setter-based
    approach which preserved omega_k; here omega_lambda is held fixed so the
    curvature (omega_k) varies between instances.
    """
   # z_values = np.linspace(0, 1, 100)
    #plt.figure()
    #base_H0 = 70
    #base_omega_lambda = 0.7  # keep omega_lambda fixed when creating new instances

   # for omega_m in (0.2, 0.3, 0.4):
        #cosmo = Cosmology(base_H0, omega_m, base_omega_lambda)  # new instance per omega_m
        #integrand_values = [cosmo.integrand(z) for z in z_values]
        #plt.plot(z_values, integrand_values, label=f"omega_m={omega_m}, omega_lambda={cosmo.omega_lambda:.2f}")

   # plt.xlabel("z")
    #plt.ylabel("Integrand")
    #plt.title("Integrand vs z for varying omega_m (new instances)")
    #plt.legend()
   # plt.grid()
   # plt.show()
...
if __name__ == "__main__":
    # create an instance and call the distance method correctly
    cosmo = Cosmology(72.0, 0.3, 0.7)
    D = cosmo.distance_rectangle(1.0, 10000)
    print(f"Distance to z=1 (rectangle, n=1000): {D:.1f} Mpc")
